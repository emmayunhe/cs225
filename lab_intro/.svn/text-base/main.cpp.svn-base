...
PNG brighten(PNG original, int amount)
{
    ...
    for (size_t yi = 0; yi < original.height(); yi++)
    {
        for (size_t xi = 0; xi < original.width(); xi++)
        {
            /// Your code here!
		int temp;
		temp = original(xi,yi)->red + amount;
		if (temp>255)
			original(xi, yi)->red=255;
		else original(xi,yi)->red = temp;

		temp = original(xi,yi)->green + amount;
		if (temp>255)
			original(xi, yi)->green=255;
		else original(xi,yi)->green = temp;

		temp = original(xi,yi)->blue + amount;
		if (temp>255)
			original(xi,yi)->blue = 255;
		else original(xi,yi)->blue = temp;
//original(xi, yi)->red+=amount>255?original(xi,yi->red=255:original(xi,yi)->red+=amount;
        }
    }
    return original;
}
